...
def _serialize_entities(
    ...
    for entity in entities:
        ...
        result.append(EntitySchema(label=entity.label, placeholder=placeholder, detections=detections))
    return result
...
def create_app(pipeline_path: str) -> Litestar:
    ...
    async def get_config() -> ConfigResponse:
        ...
        return ConfigResponse(labels=labels, placeholder_factory=factory_name,)

    @post("/v1/anonymize")
    async def anonymize(data: AnonymizeRequest) -> AnonymizeResponse:
        anonymized_text, entities = await pipeline.anonymize(
            data.text, thread_id=data.thread_id,
        )
        return AnonymizeResponse(
            anonymized_text=anonymized_text,
            entities=_serialize_entities(entities, pipeline, data.thread_id,),
        )
    ...
    return Litestar(
        route_handlers=[get_config, anonymize, deanonymize, deanonymize_entities,],
        guards=guards,
        lifespan=[lifespan],
    )
